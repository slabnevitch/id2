$(function() {
		...
		// tooltipster
			...
			$('.header-account__cart, .order-display-tooltip, .popup-registered, .header-account__favorites').tooltipster({
				...
				side: ['bottom'],
				 ...
				 functionReady: function(instance, helper) {
					// console.log('cart instance ' + helper.tooltip.classList);
					if(helper.tooltip.querySelector('.tooltip-cart') !== null){
						helper.tooltip.classList.add('tooltip-cart-origin');
	...
	// accordeon
		...
		$('.acordeon-link').click(function(e) {
			...
			if($currentItem.hasClass('acordeon-item-with-sublist')){
				...
				if($(this).hasClass('some-product__acordeon-link')){
					$currentItem
						.siblings()
		...
		$('.orders-list__toggle').click(function(e) {
				var $th = $(this),
					$thisBody = $th.closest('.orders-list__header')
											.next(),
					$thTextVal = $th.find('.orders-list__toggle-text'),
					thString = $thTextVal.text(),
					newThString;

					$thisBody.slideToggle();
					$th.toggleClass('opened');

			thString == 'Развернуть' ? newThString = "Свернуть" : newThString = "Развернуть";
			
			$thTextVal.text(newThString);
	...
	function Cabinet() {
		...
		this.hideBlockForms = function() {
			...
			$thisBlock.find('.cabinet-block__toggle.hidden').removeClass('hidden');
	...
	if($('.order-sidebar__to-fixed').length > 0 && screen.width > 780){
		...
		$(document).scroll(function() {
			...
			if($(this).scrollTop() > sidebarTop){
				...
				$sidebar.css({'left': sidebarLeft - 15, 'width': sidebarWidth});
			}else{
				
				$sidebar.removeClass('sidebar-fixed');
				$sidebar.css({'left': auto, 'width': auto});
			}
